...
func TestGenerateSeed(t *testing.T) {
	s, err := GenerateSeed(RecommendedSeedLen)
	require.NoError(t, err, "Seed generation should not fail")
	assert.Len(t, s, RecommendedSeedLen, "returned seed should have correct length")
...
func TestNewMaster(t *testing.T) {
	...
	_, err := NewMaster(s)
	require.NoError(err, "master key generation should not fail")
	// TODO: test error cases
}
...
func TestZero(t *testing.T) {
	...
	testZeroed := func(i int, testName string, key *Key) bool {
		...
		_, err = key.ECPubKey()
		if err == nil {
			t.Errorf("ECPubKey #%d (%s): wanted error, got nil", i, testName)
			return false
		}
...
func TestMaximumDepth(t *testing.T) {
	extKey, err := NewMaster([]byte(`abcd1234abcd1234abcd1234abcd1234`))
	if err != nil {
		t.Fatalf("NewMaster: unexpected error: %v", err)
	}
